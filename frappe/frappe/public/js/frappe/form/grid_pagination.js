export default class GridPagination {
	...
	get_pagination_html() {
		...
		return $(`<button class="btn btn-light btn-xs first-page"">
				<span>${__("First")}</span>
			</button>
			<button class="btn btn-light btn-xs prev-page">${frappe.utils.icon("left", "xs")}</button>
			${page_text_html}
			<button class="btn btn-light btn-xs next-page">${frappe.utils.icon("right", "xs")}</button>
			<button class="btn btn-light btn-xs last-page">
				<span>${__("Last")}</span>
			</button>`);
	}
