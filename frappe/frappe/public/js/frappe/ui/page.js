...
frappe.ui.Page = class Page {
	...
	setup_page() {
		...
		this.btn_secondary = this.page_actions.find(".btn-light");
		...
		let make_full_content = this.wrapper.find(".page-head");
		let sidebar_unsee_content = this.wrapper.find(".page-body");

		if (sidebar_unsee_content.find(".layout-side-section").length > 0) {
			console.log(true)
		} else {
			make_full_content.addClass("w-100 order-1");
			make_full_content.removeClass("col-10");
			sidebar_unsee_content.addClass("w-auto order-2");
			sidebar_unsee_content.removeClass("col-2");
		}
	}
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (frappe.utils.is_xs() || frappe.utils.is_sm()) {
					...
					make_full_content.toggleClass("col-10");
					make_full_content.addClass("w-100");
					sidebar_unsee_content.toggle();
				}
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		const button = $(`
			<button class="text-muted btn btn-default ${css_class} icon-btn">
				<img class="app-logo" src="/assets/erpnext/images/icons/${icon}.png"/>
			</button>
		`);
	...
	set_action(btn, opts) {
		...
		if (opts.label == "Discard" || opts.label == "Delete" || opts.label == "Remove") {
		...
		if (group) {
			...
			if (button.length == 0) {
				button = $(`<button data-label="${encodeURIComponent(
					label
				)}" class="btn btn-${type} ellipsis btn-lg">
					${__(label)}
				</button>`);
