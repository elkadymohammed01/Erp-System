...
export default class LinksWidget extends Widget {
	...
	set_body() {
		...
		const get_indicator_color = (item) => {
			...
			if (item.onboard) {
				return item.count ?  "mental.png" : "heartbeat.png";
			}
		...
		this.link_list = this.links.map((item) => {
			...
			return $(`<a href="${route}" class="d-flex row mord link-item ellipsis ${
				item.onboard ? "onboard-spotlight" : ""
			} ${disabled_dependent(item)}" type="${item.type}" title="${
				item.label ? item.label : item.name
			}">
					<img class="col-1 p-0 mr-1" src="/assets/frappe/images/${get_indicator_color(item)}"></span>
