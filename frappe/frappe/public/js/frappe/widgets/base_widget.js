...
export default class Widget {
	...
	make_widget() {
		this.widget = $(`<div class="card widget shadow" data-widget-name="${this.name ? this.name : ""}">
	...
	set_title(max_chars) {
		...
		if (this.icon) {
			...
			this.title_field[0].innerHTML = `<img class="col-2" src="/assets/erpnext/images/cards/${this.icon}.png "/> <span class="ellipsis" title="${title}">${title}</span>`;
		} else {
			this.title_field[0].innerHTML = `<img class="app-logo ml-2 mr-2" src="/assets/erpnext/images/cards/${this.type}.png"/><span class="ellipsis" title="${title}">${title}</span>`;
			if (max_chars) {
