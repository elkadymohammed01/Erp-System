...
frappe.views.BaseList = class BaseList {
	...
	setup_paging_area() {
		const paging_values = [5, 10, 20, 100, 500];
		this.$paging_area = $(
		...
		this.$paging_area.on("click", ".btn-paging, .btn-more", (e) => {
			...
			if ($this.is(".btn-paging")) {
				...
				this.page_length = this.selected_page_count || this.page_length;
			}
			this.refresh();
		});
	}
		...
		$(`<div class="filter-selector">
			<button class="btn btn-default btn-sm filter-button">
				<span class="filter-icon">
					<img class="app-logo" src="/assets/erpnext/images/icons/filter.png">
				</span>
