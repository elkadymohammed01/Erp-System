...
frappe.views.Workspace = class Workspace {
		...
		return $(`
			...
			item.parent_page
				...
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					...
					>
						<span class="sidebar-item-icon app-logo"><img src="/assets/erpnext/images/nav_bar_icons/${(item.icon) }.png" alt="${__(item.title)}"></span>
						<span class="sidebar-item-label text-white ml-1 mr-1">${__(item.title)}<span>
					</a>
	...
	build_sidebar_section(title, root_pages) {
		let sidebar_section = $(
			`<div class="standard-sidebar-section nested-container m-0 p-1" data-title="${title}" style="background-color:#59B179;"></div>`
		);

		let $title = $(`
			<div class="p-0 m-0" id="menuIconStyle"></div>
	...
	prepare_sidebar(items, child_container, item_container) {
		items.forEach((item) => {
			if (item.name.toLowerCase().includes("erpnext")) {
				return;
			}
	...
	append_item(item, container) {
		...
		const searchRegExp = /(\berpnext\b)/gi;
		const replaceWith = "Site";
		if (item.name.toLowerCase().includes("erpnext")) {
			item.title = item.title.replace(searchRegExp, replaceWith);
		}
	...
	add_settings_button(item, sidebar_control) {
		...
		let $button = $(`
			<div class="btn btn-light btn-xs setting-btn dropdown-btn" title="${__("Setting")}">
